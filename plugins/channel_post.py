...
from plugins.admin import sudo
from bot import Bot
...
@Bot.on_message(
    ...
    & filters.user(ADMINS)
    & ~filters.command(
        ["start", "users", "broadcast", "ping", "batch", "genlink", "promote",  "admins",  "promote",  "help",  "logs"]
    )
)
async def channel_post(client: Client, message: Message):
    reply_text = await message.reply_text("<code>Sedang Memproses Mohon Tunggu...</code>", quote=True)
    try:
        ...
        print(e)
        await reply_text.edit_text("<b>Telah Terjadi suatu kesalahan silakan cek /logs apa yang sedang terjadi...</b>")
        return
